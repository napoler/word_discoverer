# coding=utf-8
# 文件来自于 https://github.com/blmoistawinde/HarvestText/blob/01a3ae5f073f2ccf9caee4d783b6ea4cb91e37d9/harvesttext/algorithms/word_discoverer.py#L87
import re
import time
import math
import numpy as np
...
if __name__ == '__main__':
    doc = """十四是十四四十是四十，，十四不是四十，，，，四十不是十四XGBoost 和任何其他基于树的模型不能在数学上执行任何类型的大于 0 阶的外推。即它们只能外推一个常数值。在尝试将这种模型应用于非平稳时间序列时，这是一个需要考虑的巨大限制。
然而，XGBoost 仍然是一个非常有吸引力的工具，可以在具有许多特征的复杂数据中显示结构。只要您的目标是静止的，使用它来预测时间序列可能是一个不错的选择。如果不是这种情况，那么您需要预处理您的数据以确保它是或考虑将 XGBoost 与另一个负责处理趋势的模型配对。"""
    # doc = "恒大门前种两棵树都不至于这个比分恒大门前种两棵树都不至于这个比分恒大门前种两棵树都不至于这个比分"
